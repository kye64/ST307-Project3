...
*1.This data statement establishes a project 3 library in the file location.;
libname project3 "\\wolftech.ad.ncsu.edu\cos\stat\Redirect\kye\Desktop\Project 3";

*2.This data step reads in a csv file into data set winequality in project3 library.
The variable color is created based on the variable type and the variable type is then dropped.;
data project3.winequality;
	...
	drop type;
run;
*3 The t-test procedure initiates a one sided two sample paired t-test with alpha 0.05 of the dataset winequality.;
...
*4. The t-test procedure initiates a two sample t-test of the quality of wine subgroup by wine color.;
...
*5. The corr procedure initiates a correlation analysis between the variables sugar, quality, alcohol, and pH.;
...
*6 The glm procedure creates a simple linear regression model of variable quality based on the explanatory variable sugar.;
...
*7.The glm procedure creates a multiple linear regression model of response variable quality based on the explanatory variables sugar, freeS)2, totalSO2, pH, density, and alcohol. The cli option creates a prediction interval for the regression model.;
